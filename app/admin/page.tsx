...
type PaneType = "standard" | "specialty" | "french";
type StoryLevel = "1-2" | "3+";
...
export default function AdminPage() {
  ...
  const loadJobs = useCallback(async () => {
    ...
    setJobsLoading(true);
    try {
      const response = await fetch("/api/jobs", { cache: "no-store", credentials: "include" });
      if (!response.ok) {
  ...
  const loadUsers = useCallback(async () => {
    try {
      const response = await fetch("/api/users?all=true", { credentials: "include" });
      if (!response.ok) {
  ...
  async function postSiteImage(file: File, busyKey: SiteImageUploadSlot) {
    ...
    try {
      ...
      const response = await fetch("/api/admin/site-image", { method: "POST", credentials: "include", body });
  ...
  const tabs = [
    { id: "quotes", label: "Quotes", icon: FileText, section: "operations" as const },
    { id: "reps", label: "Reps", icon: Users2, section: "operations" as const },
    ...
    { id: "email", label: "Email", icon: Send, section: "config" as const },
    { id: "users", label: "Users", icon: UserCog, section: "config" as const },
  ...
  return (
    <div className="flex h-screen" style={{ background: COLORS.page }}>
      ...
      >
        <div className="px-4 py-4 border-b" style={{ borderColor: "rgba(255,255,255,0.1)" }}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-black text-sm leading-tight text-white">Mountain Springs</p>
        ...
        <div
          className="fixed bottom-0 left-0 right-0 z-50 flex border-t px-1 py-1 overflow-x-auto lg:hidden"
          style={{ background: COLORS.sidebar, borderColor: "rgba(255,255,255,0.1)" }}
        >
          {tabs.flatMap((item) => {
            const Icon = item.icon;
            const sectionButton = (
              <button
                ...
                type="button"
                onClick={() => setActiveSection(item.id)}
                className={`flex flex-col items-center gap-0.5 rounded-lg py-2 px-2 text-xs transition-all shrink-0 ${
                  activeSection === item.id ? "bg-white/5" : ""
                }`}
                style={{ color: activeSection === item.id ? COLORS.accent : "rgba(255,255,255,0.5)" }}
              >
                <Icon size={16} />
            ...
            return [
              ...
              <Link
                ...
                href="/admin/leads"
                className={`flex flex-col items-center gap-0.5 rounded-lg py-2 px-2 text-xs transition-all shrink-0 ${
                  leadsActive ? "bg-white/5" : ""
                }`}
                style={{ color: leadsActive ? COLORS.accent : "rgba(255,255,255,0.5)" }}
              >
                <FileText size={16} />
        ...
        <div className="flex-1 overflow-auto pb-20 md:pb-0">
          <div className="px-4 md:px-10 py-6">
            ...
            {activeSection === "quotes" ? (
              <>
                <div className="mb-8">
                  <p className="text-xs font-black uppercase tracking-wider mb-2" style={{ color: COLORS.accent }}>DAILY OPERATIONS</p>
                  <h1 className="text-4xl font-bold mb-3" style={{ color: COLORS.primary }}>Quotes</h1>
                  <p className="text-sm" style={{ color: COLORS.textMuted }}>Review, edit, and convert saved quotes into scheduled jobs.</p>
                </div>
                <Card className="shadow-md" style={{ borderLeft: `4px solid ${COLORS.accent}`, borderColor: COLORS.border }}>
            <CardHeader>
            ...
            {activeSection === "reps" ? (
              <>
                <div className="mb-8">
                  <p className="text-xs font-black uppercase tracking-wider mb-2" style={{ color: COLORS.accent }}>OPERATIONS</p>
                  <h1 className="text-4xl font-bold mb-3" style={{ color: COLORS.primary }}>Reps</h1>
                  <p className="text-sm" style={{ color: COLORS.textMuted }}>Manage knockers and field crew. Toggle active status, view performance.</p>
                </div>
          ...
          <div className="mb-8">
            <p className="text-xs font-black uppercase tracking-wider mb-2" style={{ color: COLORS.accent }}>OPERATIONS</p>
            <h1 className="text-4xl font-bold mb-3" style={{ color: COLORS.primary }}>Customer Reviews</h1>
            <p className="text-sm" style={{ color: COLORS.textMuted }}>Add, edit, or delete customer reviews displayed on your public site.</p>
          </div>
          <Card className="shadow-md" style={{ borderLeft: `4px solid ${COLORS.accent}`, borderColor: COLORS.border }}>
            <CardHeader>
          ...
          <div className="mb-8">
            <p className="text-xs font-black uppercase tracking-wider mb-2" style={{ color: COLORS.accent }}>DAILY OPERATIONS</p>
            <h1 className="text-4xl font-bold mb-3" style={{ color: COLORS.primary }}>Jobs</h1>
            <p className="text-sm" style={{ color: COLORS.textMuted }}>Track scheduled and active service appointments across the team.</p>
          </div>
          <div className="space-y-6">
            <Card className="shadow-md" style={{ borderLeft: `4px solid ${COLORS.accent}`, borderColor: COLORS.border }}>
              <CardHeader>
            ...
            <Card className="shadow-md bg-white text-slate-900" style={{ borderLeft: `4px solid ${COLORS.accent}`, borderColor: COLORS.border }}>
              <CardHeader>
          ...
          <>
          <div className="mb-8">
            <p className="text-xs font-black uppercase tracking-wider mb-2" style={{ color: COLORS.accent }}>MANAGEMENT</p>
            <h1 className="text-4xl font-bold mb-3" style={{ color: COLORS.primary }}>Communications</h1>
            <p className="text-sm" style={{ color: COLORS.textMuted }}>Send messages to customers and manage contacts.</p>
          </div>
          <div className="grid gap-6">
            <div className="rounded-lg bg-white p-4 shadow-sm border-l-4 border-r border-t border-b sm:p-6">
          ...
          <>
          <div className="mb-8">
            <p className="text-xs font-black uppercase tracking-wider mb-2" style={{ color: COLORS.accent }}>SETUP</p>
            <h1 className="text-4xl font-bold mb-3" style={{ color: COLORS.primary }}>Users</h1>
            <p className="text-sm" style={{ color: COLORS.textMuted }}>Create and manage admin, rep, and tech user accounts.</p>
          </div>
          <UsersAdminPanel />
          </>
          ) : null}
          ...
          <div className="mb-8">
            <p className="text-xs font-black uppercase tracking-wider mb-2" style={{ color: COLORS.accent }}>DAILY OPERATIONS</p>
            <h1 className="text-4xl font-bold mb-3" style={{ color: COLORS.primary }}>Schedule</h1>
          ...
          <div className="mb-8">
            <p className="text-xs font-black uppercase tracking-wider mb-2" style={{ color: COLORS.accent }}>SETUP</p>
            <h1 className="text-4xl font-bold mb-3" style={{ color: COLORS.primary }}>Pricing</h1>
            <p className="text-sm" style={{ color: COLORS.textMuted }}>Set service pricing by type and home size, plus add-on rates.</p>
          </div>

          <Card className="shadow-md" style={{ borderLeft: `4px solid ${COLORS.accent}` }}>
            <CardHeader>
              <CardTitle>Service Pricing by Home Size</CardTitle>
              <CardDescription>Set base prices for each service type and bedroom count.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              {Object.entries(pricing.services).map(([serviceId, service]) => (
                <div key={serviceId}>
                  <h3 className="text-sm font-semibold mb-3">{service.label}</h3>
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
                    {Object.entries(service.prices).map(([sizeId, price]) => {
                      const sizeLabel = { studio: "Studio", "1br": "1BR", "2br": "2BR", "3br": "3BR", "4br": "4BR", "5br_plus": "5BR+" }[sizeId] || sizeId;
                      return (
                        <div key={sizeId} className="space-y-1">
                          <Label htmlFor={`price-${serviceId}-${sizeId}`} className="text-xs text-slate-600">
                            {sizeLabel}
                          </Label>
                          <Input
                            id={`price-${serviceId}-${sizeId}`}
                            type="number"
                            min={0}
                            step="0.01"
                            value={price}
                            onChange={(event) => {
                              setPricing((prev) => ({
                                ...
                                services: {
                                  ...prev.services,
                                  [serviceId]: {
                                    ...prev.services[serviceId as keyof typeof prev.services],
                                    prices: {
                                      ...prev.services[serviceId as keyof typeof prev.services].prices,
                                      [sizeId]: Number(event.target.value) || 0,
                                    },
                                  },
                                },
                              }));
          ...
          <Card className="shadow-lg border border-slate-200 bg-white text-slate-900">
            <CardHeader>
              ...
              <CardDescription>Optional add-ons customers can select.</CardDescription>
            </CardHeader>
            <CardContent>
                      ...
                      <Input
                        ...
                        onChange={(event) => {
                          setPricing((prev) => ({
                            ...
                            addons: {
                              ...prev.addons,
                              [addonId]: Number(event.target.value) || 0,
                            },
                          }));
                        }}
                        className="h-10"
          ...
          <Card className="shadow-lg border border-slate-200 bg-white text-slate-900">
            <CardHeader>
              ...
              <CardDescription>Minimum charge floor for any service.</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <>
          <div className="mb-8">
            <p className="text-xs font-semibold uppercase tracking-widest text-[#f59e0b] mb-1">CONFIGURATION</p>
            <h1 className="text-3xl font-bold text-slate-900">Business</h1>
            <p className="text-sm text-slate-500 mt-1">Manage public business info, hours, and brand imagery.</p>
          </div>
          <Card className="shadow-lg border border-slate-200 bg-white text-slate-900">
            <CardHeader>
                        ...
                        onClick={() =>
                          setPublicBusiness((current) =>
                            normalizePublicBusinessConfig(
                              {
                                ...current,
                                randomBackgroundImageUrls: current.randomBackgroundImageUrls.filter((_: string, i: number) => i !== index),
                              },
                              scheduleWindows.rep,
                            ),
                          )
          ...
          <div className="mb-8">
            ...
            <p className="text-sm text-slate-500 mt-1">Set prices for optional service add-ons.</p>
          </div>
          <Card className="shadow-lg border border-slate-200 bg-white text-slate-900">
            <CardHeader>
          ...
          <>
          <div className="mb-8">
            <p className="text-xs font-semibold uppercase tracking-widest text-rose-600 mb-1">DANGER ZONE</p>
            <h1 className="text-3xl font-bold text-slate-900">Reset Database</h1>
            <p className="text-sm text-slate-500 mt-1">Clear all operational data and restore app config to defaults.</p>
          </div>
